...
from grid_world import plot_grid_world
...
def get_trajectory(
    ...
    """
    Plan a trajectory from start_position to end_position.
    ...
    grid : np.ndarray
        Occupancy grid. 0 is free, 1 is occupied.

    allow_diagonal : bool
        If True, use 8-connected motion. If False, use 4-connected motion.

    waypoint_stride : int
        Keep every Nth point from the dense path to create sparse waypoints.
    ...
    dict
        {
            ...
            "goal": end_position,
            "path": dense A* path,
            "waypoints": sparse waypoint list,
    ...
    waypoints = sparsify_path(path, stride=waypoint_stride)
...
def main() -> None:
    ...
    start = (100, 100)
    room_name = "room_1"
    path = plan_to_room(start, room_name, grid, rooms, allow_diagonal=True)
